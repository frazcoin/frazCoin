...
#if defined(NDEBUG)
# error "frazCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd18551253b35e9037c98b10aaaa82eb735b95ace14bca251d8c402765df1bbe0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FrazCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FrazCoin Signed Message:\n";
...
{
    // FrazCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy;
    if(nHeight == 1)
        nSubsidy = 200000 * COIN; // Premine for development, promotion etc
    else
    {
        nSubsidy = 1000 * COIN; // 1000 FrazCoins
        nSubsidy >>= ((nHeight-1/*Ignore the premined block*/) / 5000); // Subsidy is cut in half every 5000 blocks
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FrazCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x97a2b64a2c733a92c89fd8463b8d7fbfe8ea483cbb0453c17fc9de2fac00879a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2017 July, the creation of new cryptocurrency frazCoin.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0471bbc7d3a4baee7d22426cd0c7d059fea97fb40e86fabeff2d6969d7b67aba2bdc8c9ad85cc58e9b4c0d32afe57ce8b48f4e9561ca108ab80a487d8417d42f76") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1500036545;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 956914;

        if (fTestNet)
        {
            block.nTime    = 1500036546;
            block.nNonce   = 1143203;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5ecb6c09d7fe5f6d4b3ae2e8cd58cf67898ab71c8ba5d5a39ee164bfd3bd4458"));
...
unsigned char pchMessageStart[4] = { 0x46, 0x52, 0x41, 0x5A };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FrazCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FrazCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static frazCoinMiner(CWallet *pwallet)
{
    printf("FrazCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("FrazCoin-miner");
    ...
    try { loop {
        ...
        printf("Running FrazCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FrazCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&frazCoinMiner, pwallet));
}
